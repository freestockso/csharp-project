// chstoolpro.cpp : 定义 DLL 应用程序的导出函数。
//
...
__declspec(dllexport) int WINAPI IsValidSerial(CALCINFO* pData){
	if(pData->m_pfParam1){
		if (pData->m_nParam1Start>=0&&pData->m_nNumData>0){//参数1为序列
			int pos=pData->m_nNumData-1;
			if (pData->m_pfParam1[pData->m_nParam1Start]==NULL) pData->m_pResultBuf[pos]=0;
			else pData->m_pResultBuf[pos]=1;
			return pos;
		}
	}
	return -1;
